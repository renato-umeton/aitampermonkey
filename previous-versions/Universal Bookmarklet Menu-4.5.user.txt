...
(function() {
    ...
    const basketBookmarklets = [{
        ...
        code: function() {
            ...
            // No extra prompt — copy the raw basket URLs so the user can paste them
            // (CTRL+V) straight into NotebookLM as sources.
            const urlList = basket.map((item) => item.url).join("\n");

            const openNotebookLM = function() {
                showCustomGeminiNotification("Material copied — paste it with CTRL+V. Opening NotebookLM now…");
...
<body>
    ...
    <\/script>
        ...
        name: "📓 NotebookLM", // Copies context to the clipboard, then opens NotebookLM for pasting
        code: function() {
            var selection = window.getSelection().toString().trim();
            // No extra prompt — copy the raw material (selected text or page URL) so the
            // user can paste it (CTRL+V) straight into NotebookLM as a source.

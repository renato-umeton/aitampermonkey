...
// @version      4.3
// @description  Floating AI bookmarklet menu with auto-updates. Quick access to ChatGPT, Claude, Gemini, NotebookLM, Kagi, and page utilities.
...
// @updateURL    https://raw.githubusercontent.com/renato-umeton/aitampermonkey/main/universal-bookmarklet-menu.user.js
// @downloadURL  https://raw.githubusercontent.com/renato-umeton/aitampermonkey/main/universal-bookmarklet-menu.user.js
...
(function() {
    ...
    function getSettings() {
        try {
            const defaults = {
                position: 'right',       // 'right', 'left', or 'custom'
                customX: null,           // custom X position (px from left)
                customY: null,           // custom Y position (px from top)
                darkThemeFollowing: true  // true = follow system, false = always light
            };
            const stored = GM_getValue('menu_settings', '{}');
            return Object.assign(defaults, JSON.parse(stored));
        } catch (e) {
            console.error('Error reading settings:', e);
            return { position: 'right', customX: null, customY: null, darkThemeFollowing: true };
        }
    }

    function saveSettings(settings) {
        try {
            GM_setValue('menu_settings', JSON.stringify(settings));
        } catch (e) {
    ...
    const basketBookmarklets = [{
        ...
        code: function() {
            ...
            // NotebookLM has no URL-prefill API, so copy the basket URLs to the
            // clipboard for the user to paste in as sources, then open NotebookLM.
            const urlList = basket.map((item) => item.url).join("\n");

            const openNotebookLM = function() {
                showStatusMessage("Basket URLs copied — paste them as NotebookLM sources.", 3000);
                window.open("https://notebooklm.google.com/", "_blank");
            };

            navigator.clipboard.writeText(urlList).then(openNotebookLM).catch(function(err) {
                console.error('Failed to copy basket URLs for NotebookLM:', err);
                openNotebookLM();
            });
        }
    }];
    const regularBookmarklets = [{
        ...
        name: "🖥️ View Source",
        code: function() {
            // Fetch the current page's HTML source and display it in a new tab
            const currentUrl = location.href;
            ...
            // Helper function to apply syntax highlighting to HTML
            function highlightHtml(escapedHtml) {
                // Highlight HTML comments
                let highlighted = escapedHtml.replace(/(&lt;!--[\s\S]*?--&gt;)/g, '<span class="hl-comment">$1</span>');

                // Highlight doctype
                highlighted = highlighted.replace(/(&lt;!DOCTYPE[^&]*&gt;)/gi, '<span class="hl-doctype">$1</span>');

                // Highlight tags, attributes, and strings
                highlighted = highlighted.replace(/(&lt;\/?)([\w-]+)([\s\S]*?)(&gt;)/g, function(match, open, tagName, rest, close) {
                    // Process attributes within the tag
                    let processedRest = rest.replace(/([\w-]+)(=)(&quot;|&#039;)([\s\S]*?)(\3)/g,
                        '<span class="hl-attr">$1</span>$2<span class="hl-string">$3$4$5</span>');
                    return open + '<span class="hl-tag">' + tagName + '</span>' + processedRest + close;
                });
            ...
            function getPageDate() {
                // Try to get last modified date from document
                const lastModified = document.lastModified;
                if (lastModified && lastModified !== '0') {
                    const date = new Date(lastModified);
                    // Check if the date is valid and not the default
            ...
            showStatusMessage("Fetching page source...", 2000);

            // Use GM_xmlhttpRequest to fetch the page source
            GM_xmlhttpRequest({
                method: 'GET',
                url: currentUrl,
                onload: function(response) {
                    try {
                        ...
                        const escapedSource = escapeHtml(sourceCode);
                        const highlightedSource = highlightHtml(escapedSource);

                        // Create a new window/tab with the source code
                        const newWindow = window.open('', '_blank');
                        if (newWindow) {
                            // Create a properly formatted HTML page to display the source
                            const displayHtml = `<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>Source: ${escapedTitle}</title>
    <style>
        * {
        ...
        pre.plain-text .hl-tag,
        pre.plain-text .hl-attr,
        pre.plain-text .hl-string,
        pre.plain-text .hl-comment,
        pre.plain-text .hl-doctype {
            color: inherit;
            font-weight: inherit;
            font-style: inherit;
        }
...
<body>
    <div class="header">
        ...
        <div class="header-info">
            ...
            <p><strong>${pageDate.type}:</strong> ${pageDate.date} at ${pageDate.time}</p>
        </div>
        <div class="controls">
            <div class="toggle-group">
                ...
                <label class="toggle-label" for="syntaxToggle">Syntax Highlighting</label>
            </div>
            <div class="toggle-group">
                <label class="toggle-switch">
                ...
                <label class="toggle-label" for="wrapToggle">Line Wrap</label>
            </div>
        </div>
    </div>
    <pre id="sourceCode" class="wrap-lines">${highlightedSource}</pre>
    <script>
        ...
        syntaxToggle.addEventListener('change', function() {
            if (this.checked) {
                sourceCode.classList.remove('plain-text');
            } else {
                sourceCode.classList.add('plain-text');
            }
        });

        wrapToggle.addEventListener('change', function() {
            if (this.checked) {
                sourceCode.classList.remove('no-wrap');
                sourceCode.classList.add('wrap-lines');
            } else {
                sourceCode.classList.remove('wrap-lines');
                sourceCode.classList.add('no-wrap');
            }
        });
    <\/script>
</body>
                            ...
                            // Note: document.write() is safe here as all dynamic content is escaped via escapeHtml()
                            newWindow.document.write(displayHtml);
                            newWindow.document.close();
                            showStatusMessage("Source code opened in new tab!", 2000);
                        } else {
                            showStatusMessage("Failed to open new tab. Check popup blocker.", 3000, true);
                        }
                    } catch (error) {
                        console.error("Error displaying source code:", error);
                        showStatusMessage("Error displaying source code: " + error.message, 3000, true);
                    }
                },
                onerror: function(error) {
                    console.error("Error fetching source code:", error);
                    showStatusMessage("Error fetching page source. Check console.", 3000, true);
                },
                ontimeout: function() {
                    showStatusMessage("Request timeout - failed to fetch source code", 3000, true);
                },
        ...
        name: "📓 NotebookLM", // Copies context for use as a NotebookLM source, then opens NotebookLM
        code: function() {
            var selection = window.getSelection().toString().trim();
            // NotebookLM has no URL-prefill API, so copy the context (selected text
            // or the page URL) to the clipboard for pasting as a source, then open it.
            var textToCopy = selection ? selection : location.href;

            const openNotebookLM = function() {
                showStatusMessage(selection ?
                    "Selected text copied — paste it as a NotebookLM source." :
                    "Page URL copied — paste it as a NotebookLM source.", 3000);
                window.open("https://notebooklm.google.com/", "_blank");
            };

            navigator.clipboard.writeText(textToCopy).then(openNotebookLM).catch(function(err) {
                console.error('Tampermonkey Script: Failed to copy text for NotebookLM: ', err);
                openNotebookLM();
            });
        }
    }, {
        ...
        code: async function() {
            try {
                ...
                const geminiPrompt = `You are a JavaScript code generator. The user wants to implement the following on a web page:
...
Current page title: ${document.title}`;
    ...
    function applyPositionSetting() {
        ...
        if (pos === 'left') {
            ...
            menuTrigger.style.left = currentSettings.customX + 'px';
            menuTrigger.style.top = currentSettings.customY + 'px';
        } else {
            // default: right
    ...
    function applyThemeSetting() {
        if (currentSettings.darkThemeFollowing) {
            document.documentElement.classList.remove('custom-bm-force-light');
        } else {
            document.documentElement.classList.add('custom-bm-force-light');
        }
    }
    ...
    function saveCustomPosition() {
        if (currentSettings.position === 'custom') {
            ...
            currentSettings.customX = rect.left;
            currentSettings.customY = rect.top;
    ...
    function handleDragEnd() {
        ...
        if (dragState.isActuallyDragging) {
            ...
            saveCustomPosition(); // Persist position if custom mode
        }
    ...
    multiPageActionsMenuItem.innerHTML = '<span class="submenu-arrow">▾</span> 🛒 multi-page, etc.';
    ...
    // Override CSS hover behavior with JavaScript to only show submenu on left half
    let submenuVisible = false;

    multiPageActionsMenuItem.addEventListener('mouseenter', () => {
        // Don't do anything on enter - wait for mousemove
    });

    multiPageActionsMenuItem.addEventListener('mousemove', (event) => {
        const rect = multiPageActionsMenuItem.getBoundingClientRect();
        const mouseX = event.clientX - rect.left;
        const halfWidth = rect.width / 2;

        if (mouseX <= halfWidth) {
            // Left half - show submenu and allow hover background
            if (!submenuVisible) {
                submenuContainer.style.display = 'block';
                submenuVisible = true;
            }
            multiPageActionsMenuItem.style.backgroundColor = '';
            multiPageActionsMenuItem.style.pointerEvents = '';
        } else {
            // Right half - hide submenu and remove hover background
            if (submenuVisible) {
                submenuContainer.style.display = 'none';
                submenuVisible = false;
            }
            multiPageActionsMenuItem.style.backgroundColor = 'transparent';
        }
    });

    multiPageActionsMenuItem.addEventListener('mouseleave', (event) => {
        ...
        // Reset background color when leaving the item
        multiPageActionsMenuItem.style.backgroundColor = '';
    });
    ...
    // Add "Hide Menu App" action to the submenu
    const hideAppMenuItem = document.createElement('a');
    hideAppMenuItem.href = '#';
    hideAppMenuItem.className = 'custom-bm-menu-item';
    hideAppMenuItem.textContent = '🚫 Hide Menu App';
    hideAppMenuItem.addEventListener('click', function(event) {
        event.preventDefault();
        event.stopPropagation();

        document.getElementById('custom-bm-menu-trigger').style.display = 'none';

        document.getElementById('custom-bm-menu-container').style.display = 'none';
    ...
    posLabel.className = 'custom-bm-option-label';
    posLabel.textContent = 'Position';
    optionsPane.appendChild(posLabel);

    const posGroup = document.createElement('div');
    posGroup.className = 'custom-bm-option-group';
    optionsPane.appendChild(posGroup);

    ['right', 'left', 'custom'].forEach(pos => {
        const btn = document.createElement('button');
        btn.className = 'custom-bm-option-btn' + (currentSettings.position === pos ? ' active' : '');
        ...
        btn.addEventListener('click', (e) => {
            ...
            if (pos === 'custom') {
                // Save current dragged position as custom
                const rect = menuTrigger.getBoundingClientRect();
                currentSettings.customX = rect.left;
                currentSettings.customY = rect.top;
            }
            ...
            posGroup.querySelectorAll('.custom-bm-option-btn').forEach(b => b.classList.remove('active'));
    ...
    customHint.className = 'custom-bm-option-hint';
    customHint.textContent = 'Drag 📟 to desired position; it will be saved.';
    customHint.style.display = currentSettings.position === 'custom' ? 'block' : 'none';
    optionsPane.appendChild(customHint);

    // Dark theme following setting
    const themeLabel = document.createElement('div');
    themeLabel.className = 'custom-bm-option-label';
    themeLabel.textContent = 'Dark theme';
    themeLabel.style.marginTop = '10px';
    optionsPane.appendChild(themeLabel);

    const themeGroup = document.createElement('div');
    themeGroup.className = 'custom-bm-option-group';
    optionsPane.appendChild(themeGroup);

    const themeToggleLabel = document.createElement('label');
    themeToggleLabel.className = 'custom-bm-theme-toggle';

    const themeCheckbox = document.createElement('input');
    themeCheckbox.type = 'checkbox';
    themeCheckbox.checked = currentSettings.darkThemeFollowing;
    themeCheckbox.addEventListener('change', (e) => {
        e.stopPropagation();
        currentSettings.darkThemeFollowing = themeCheckbox.checked;
        saveSettings(currentSettings);
        applyThemeSetting();
    });

    const themeSlider = document.createElement('span');
    themeSlider.className = 'custom-bm-theme-slider';

    const themeText = document.createElement('span');
    themeText.className = 'custom-bm-theme-text';
    themeText.textContent = 'Follow system dark mode';

    themeToggleLabel.appendChild(themeCheckbox);
    themeToggleLabel.appendChild(themeSlider);
    themeToggleLabel.appendChild(themeText);
    themeGroup.appendChild(themeToggleLabel);

    // Toggle options pane visibility
    ...
    versionLabel.className = 'custom-bm-version-label';
    versionLabel.textContent = `v${SCRIPT_VERSION} · ${SCRIPT_DATE}`;
    menuContainer.appendChild(versionLabel);
    ...
    z-index: 100000;
        ...
        /* Options Pane */
        .custom-bm-options-toggle {
            font-size: 14px;
        }
        .custom-bm-options-pane {
            padding: 8px 20px 12px;
        }
        .custom-bm-option-label {
            font-size: 12px;
            font-weight: 600;
            color: var(--item-text);
            margin-bottom: 6px;
        ...
        .custom-bm-option-group {
            display: flex;
            gap: 6px;
            flex-wrap: wrap;
            align-items: center;
        }
        .custom-bm-option-btn {
            padding: 5px 12px;
            border: 1px solid var(--menu-border);
            border-radius: 6px;
        ...
        .custom-bm-option-btn:hover {
            background-color: var(--item-hover-bg);
        }
        .custom-bm-option-btn.active {
            background-color: #4285F4;
            color: #fff;
            border-color: #4285F4;
        }
        .custom-bm-option-hint {
            font-size: 11px;
            color: var(--item-text);
            opacity: 0.6;
            margin-top: 4px;
        ...
        .custom-bm-theme-toggle input {
            position: absolute;
            opacity: 0;
            width: 0;
            height: 0;
        }
        .custom-bm-theme-slider {
            position: relative;
        ...
        .custom-bm-theme-toggle input:checked + .custom-bm-theme-slider {
            background-color: #4285F4;
        }
        .custom-bm-theme-toggle input:checked + .custom-bm-theme-slider::before {
            transform: translateX(16px);
        }
        .custom-bm-theme-text {
            font-size: 13px;
            color: var(--item-text);
        }
        ...
        /* Dark Theme */
        @media (prefers-color-scheme: dark) {
            :root:not(.custom-bm-force-light) {
                --menu-bg: #2b2b2b;
                --menu-border: #444;

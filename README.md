...
| ------- | ---- | ------------- | ------- |
| 4.6 | `universal-bookmarklet-menu.user.js` | Fixed broken Harvard proxy link (#29) | - "♓ Get it at Harvard!" now redirects straight through the OpenAthens redirector (`https://go.openathens.net/redirector/library.harvard.edu?url=`) with the current page URL, removing the broken double-wrapped `ezp.prod1.hul.harvard.edu` login layer introduced in v4.5 |
| 4.5 | `universal-bookmarklet-menu.user.js` | Updated Harvard proxy link (#26) | - "♓ Get it at Harvard!" now uses the new OpenAthens redirector (`https://go.openathens.net/redirector/library.harvard.edu`) wrapping the updated `ezp.prod1.hul.harvard.edu` login URL, replacing the retired `ezp-prod1.hul.harvard.edu` proxy |
